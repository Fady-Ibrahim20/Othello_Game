...
char board[8][8];
 ...
 AlphaBetaPruningAlgorithm algorithm;
...
////max depth 5 to 6
    ...
    while (true) {
        ...
        char player;
        while (flag) {
            pair<int,pair<int,int>> res=algorithm.doAlgorithm(othelloGame,board,2,'X');
            int val= res.first ;
...
//            cin >> row >> col;
        ...
        while(flag)
        {
           // pair<int,pair<int,int>> res=MiniMaxAlgorithm(board,2,'N'); 42 13 60 24
            pair<int,pair<int,int>> res=algorithm.doAlgorithm(othelloGame,board,7,'N');
            int val= res.first ;
